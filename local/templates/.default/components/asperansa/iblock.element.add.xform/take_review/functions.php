...
if (!function_exists("ShowFormHeaderField2"))
{
    function ShowFormHeaderField2($propertyID, $arResult, $arParams) {?>
...
switch ($INPUT_TYPE):
    case "USER_TYPE":
        ...
        for ($i = 0; $i<$inputNum; $i++)
        {
			...
			if ($propertyID == 24)
				echo str_replace("(не установлено)", "Выберите врача", call_user_func_array($arResult["PROPERTY_LIST_FULL"][$propertyID]["GetPublicEditHTML"],
				 array(
		   ...
		   if ($propertyID == 31)
				echo str_replace("(не установлено)", "Выберите услугу", call_user_func_array($arResult["PROPERTY_LIST_FULL"][$propertyID]["GetPublicEditHTML"],
                array(
                    $arResult["PROPERTY_LIST_FULL"][$propertyID],
                    array(
                        "VALUE" => $value,
    ...
    case "N":
        for ($i = 0; $i<$inputNum; $i++)
        {
            ...
            ?>
            <input placeholder="<?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?>"
    ...
    case "F":
        for ($i = 0; $i<$inputNum; $i++)
        {
           ...
           <input id="FILE_<?=$propertyID?>_<?=$arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"]?>" type="file" size="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>"  
			name="PROPERTY_FILE_<?=$propertyID?>_<?=$arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] ? $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] : $i?>" <?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?>required<?endif;?>/>
                <div class="but_file">Загрузить фото</div>
                <?

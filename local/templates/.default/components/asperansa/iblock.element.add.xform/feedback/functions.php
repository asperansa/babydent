...
if (!function_exists("ShowFormHeaderField3"))
{
    function ShowFormHeaderField3($propertyID, $arResult, $arParams) {?>
...
switch ($INPUT_TYPE):
    ...
    case "T":
        for ($i = 0; $i<$inputNum; $i++)
        {
            ...
            ?>
            <textarea class="form_textarea" placeholder="<?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?>"
    ...
    case "N":
        for ($i = 0; $i<$inputNum; $i++)
        {
            ...
            ?>
            <input class="form_input <?=($propertyID == 34)? "masked_phone" : ""?>" placeholder="<?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?>"
				type="<?=($propertyID == 35)? "email" : "text"?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>" <?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?>required<?endif;?>/><?

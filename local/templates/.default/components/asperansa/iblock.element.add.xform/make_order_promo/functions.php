...
if (!function_exists("ShowFormField1"))
{
    function ShowFormHeaderField1($propertyID, $arResult, $arParams) {?>
...
switch ($INPUT_TYPE):
    ...
    case "N":
        for ($i = 0; $i<$inputNum; $i++)
        {
            ...
            ?>
            <input class="form_input <?if ($propertyID == 1):?>masked_phone<?endif;?>" placeholder="<?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?>"
